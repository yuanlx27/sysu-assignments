...
- 按照图片中的要求，完成《计算机图形学》作业三；
- 使用 CMake 的 FetchContent 模块拉取需要的库如 GLFW、IMGUI 等，尽可能不使用 GLM 和 GLAD；
- 项目代码写在 `impl` 子文件夹中，所有代码只写在 `impl/CMakeLists.txt` 和 `impl/src/main.cpp` 文件中，不要创建其他的文件；
- 不要操作 Git。